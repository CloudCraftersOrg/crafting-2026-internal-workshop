"""cli.py – Conversational CLI entrypoint for the workshop agent (Strands).
...
Usage:
    workshop-agent [OPTIONS]
    python -m workshop_agent [OPTIONS]
"""
...
from workshop_agent import __version__
from workshop_agent.agent import build_system_prompt, create_agent
from workshop_agent.config import load_config
from workshop_agent.export import BundleBuilder, export_bundle
from workshop_agent.logging import AgentLogger, LogContext
from workshop_agent.memory import AgentMemory
...
\\* Requiere persistir turnos en AgentCore Memory — ver el diferenciador
"Conversational memory across sessions" en el `README.md`.

Escribe cualquier otra cosa para conversar con el agente.
"""
...
async def _run_session(
    ...
    console.print(
        Panel.fit(
            f"[bold cyan]Workshop Agent[/bold cyan]  v{__version__}\n"
    ...
    try:
        while True:
            try:
                raw = console.input("[bold cyan]Crafty>[/bold cyan] ").strip()
            ...
            console.print(Panel(Markdown(answer), title="Crafty", border_style="green"))
...
@click.version_option(__version__, prog_name="workshop-agent")
def main(
    model: Optional[str],
    export_dir: str,
) -> None:
    """Workshop Agent – Customizable AI agent via Strands + MCP."""
    try:
    ...
    console.print(
        f"[dim]Modelo: [bold]{config.effective_model_id}[/bold][/dim]"
    )
